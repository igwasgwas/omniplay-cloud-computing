...
export default function OmniPlayApp() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white font-sans flex overflow-hidden">
      {/* Sidebar Navigation */}
      <nav className="w-20 lg:w-64 bg-gray-900 border-r border-gray-800 flex flex-col justify-between py-6">
        <div>
...
function PlayerDashboard() {
  const games = [
    { id: 1, title: 'Cyberpunk 2077', status: 'Ready to Play' },
    { id: 2, title: 'EA FC 25', status: 'Playing Now', active: true },
    { id: 3, title: 'Elden Ring', status: 'Update Available' },
  ];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {games.map(game => (
          <div key={game.id} className={`group relative bg-gray-900 rounded-xl overflow-hidden border ${game.active ? 'border-cyan-500/50 shadow-[0_0_20px_rgba(34,211,238,0.15)]' : 'border-gray-800 hover:border-gray-700'} transition-all duration-300 hover:-translate-y-1`}>
            <div className="w-full h-48 bg-gray-800 flex items-center justify-center opacity-80 group-hover:opacity-100 transition-opacity">
               <span className="text-gray-500 font-bold tracking-widest uppercase">{game.title}</span>
            </div>
            <div className="p-5">
              ...
              <button className={`w-full mt-4 py-2 rounded-lg font-medium transition-colors ${
                ...
                {game.active ? 'Resume Session' : 'Play Now'}
              </button>
            </div>
          </div>
        ))}
...
// --- 2. Tactical Companion Dashboard ---
    ...
    <div className="h-full flex items-center justify-center p-8 bg-gray-950/50">
      ...
      <div className="w-[375px] h-[812px] bg-gray-900 border-[8px] border-gray-950 rounded-[3rem] shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden relative flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-6 custom-scrollbar">
          ...
          <button className="w-full mt-6 py-3 bg-cyan-500/10 border border-cyan-500/50 text-cyan-400 font-bold rounded-xl hover:bg-cyan-500 hover:text-gray-950 transition-all shadow-[0_0_15px_rgba(34,211,238,0.2)]">
            Apply Quick Tactics
          </button>
        </div>
      </div>
          ...
          {exportOpen && (
            <div className="absolute right-0 mt-2 w-48 bg-gray-800 border border-gray-700 rounded-lg shadow-xl overflow-hidden z-20 origin-top-right">
              <button 
                ...
                className="w-full flex items-center gap-3 px-4 py-3 text-sm text-left hover:bg-gray-700 transition-colors"
              >
              ...
              <button 
                onClick={() => setExportOpen(false)}
                className="w-full flex items-center gap-3 px-4 py-3 text-sm text-left hover:bg-gray-700 transition-colors border-t border-gray-700/50"
              >
